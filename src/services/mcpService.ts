...
import { QAEntry, UploadedFile } from '../types';
import { CustomerCase, CreateCaseRequest, UpdateCaseRequest, SearchCasesRequest } from '../types/customerCase';
import { createS3ManualProvider, ManualProvider } from './manualProvider';
import { createS3QAProvider, QAProvider } from './qaProvider';
...
Matches across question, answer, and category fields.`,
      ...
      async (args) => {
        ...
        if (args.intelligent !== false) {
          // 使用智能搜尋 (預設)
          entries = await (this.qaProvider as any).intelligentSearch(args.query, args.limit || 5);
        } else {
          // 使用基本搜尋
...
Use this when you need to find specific information from a manual without downloading the entire file.`,
      ...
      async (args) => {
        ...
        // 使用搜尋服務找相關段落
        const results = contentExtractionService.searchChunks(
          fileContent.chunks,
          args.query,
          args.limit || 5
        );
...
This searches the entire manual database using AI-powered semantic understanding.`,
      ...
      async (args) => {
        try {
          ...
          const results = await vectorIndexService.searchAllManuals(
            args.query,
            args.k || 5,
            args.minScore || 0.5
          );
    ...
    this.server.registerTool(
      ...
      async (args) => {
        try {
          const customerCase = await caseService.createCase(args as CreateCaseRequest);
          return {
            content: [
              {
                type: 'text',
    ...
    this.server.registerTool(
      ...
      async (args) => {
        try {
          ...
          const customerCase = await caseService.updateCase(
            caseId,
            update as UpdateCaseRequest,
            actor || 'system'
          );
    ...
    this.server.registerTool(
      ...
      async (args) => {
        try {
          const cases = await caseService.searchCases(args as SearchCasesRequest);
        ...
        try {
          ...
          const text = `Found ${topRecommendations.length} recommended solutions:\n\n` +
            topRecommendations.map((rec, idx) => 
              `${idx + 1}. [Score: ${rec.score.toFixed(3)}] ${rec.qa.question}\n` +
    ...
    this.server.registerTool(
      ...
      async (args) => {
        try {
          ...
          // 簡單的關鍵字匹配過濾
          const query = args.query.toLowerCase();
          const matchedManuals = allManuals.filter(m => 
      ...
      async (args) => {
        try {
          ...
          for (const c of cases) {
            ...
            const score = this.cosineSimilarity(
              queryEmbedding.data[0].embedding,
              caseEmbedding.data[0].embedding
            );
        ...
        try {
          ...
          const text = `Common Issues Analysis (${period}):\n\n` +
            ...
            topIssues.map(([issue, count], idx) => 
              `${idx + 1}. ${issue}: ${count} cases (${(count / filteredCases.length * 100).toFixed(1)}%)`
            ).join('\n') +
            `\n\nTop Device Models:\n` +
            topDevices.map(([device, count], idx) => 
              `${idx + 1}. ${device}: ${count} cases`
            ).join('\n');
    ...
    this.server.registerTool(
      ...
      async (args) => {
        try {
          ...
          // 按優先級分組統計
          const byPriority: Record<string, { count: number; avgTime: number }> = {};
          ...
          const text = `Resolution Trends Analysis (${period}):\n\n` +
            ...
            Object.entries(byPriority)
              ...
              .map(([priority, stats]) => 
                `- ${priority}: ${toHours(stats.avgTime)}h avg (${stats.count} cases)`
              ).join('\n');
...
If any aspect is lacking, improve before responding.`;
  ...
  private registerResources() {
    ...
    const manualsTemplate = new ResourceTemplate(
      'waferlock://manuals/{id}',
      {
        list: async () => {
          const manuals = await this.manualProvider.listManuals();
          return {
    ...
    this.server.registerResource(
      ...
      async (uri, variables) => {
        const fileId = variables.id as string;
    ...
    const qaTemplate = new ResourceTemplate(
    ...
    this.server.registerResource(
      ...
      async (uri, variables) => {
        const categoryOrId = decodeURIComponent(variables.categoryOrId as string);
  ...
  private registerPrompts() {
    ...
    this.server.registerPrompt(
      ...
      async (args) => {
        const model = args.device_model as string;
        const errorCode = args.error_code as string || 'N/A';
        const description = args.customer_description as string || 'Not provided';
...
Please begin the troubleshooting process.`;
    ...
    this.server.registerPrompt(
      ...
      async (args) => {
        const model = args.device_model as string;
        const type = args.installation_type as string || 'new_installation';
        const env = args.environment as string || 'Standard environment';
...
Begin with the pre-installation checklist.`;
    ...
    this.server.registerPrompt(
      ...
      async (args) => {
        const model = args.device_model as string;
        const type = args.maintenance_type as string;
        const hours = args.operating_hours as string || 'Normal usage';
...
Present the checklist in an easy-to-follow format.`;
    ...
    this.server.registerPrompt(
      ...
      async (args) => {
        const model = args.device_model as string || 'All models';
        const category = args.issue_category as string || 'all categories';
...
Generate the common issues list now.`;
    ...
    this.server.registerPrompt(
      ...
      async (args) => {
        const serial = args.serial_number as string || 'Not provided';
        const purchaseDate = args.purchase_date as string || 'Not provided';
        const issueType = args.issue_type as string || 'Not specified';
